...
const palette: Record<Variant, { fg: string; border: string; bg: string }> = {
  default: { fg: "var(--color-text)", border: "var(--color-border-bright)", bg: "var(--color-bg-card)" },
  ...
  danger: { fg: "#ffffff", border: "var(--color-danger)", bg: "var(--color-danger)" },
...
export function BracketButton({
  ...
  return (
    <button
      {...rest}
      className={
        "inline-flex items-center gap-1.5 px-3.5 py-1.5 text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed hover:brightness-110 active:translate-y-px " +
        className
      }
